...
import br.com.fiap.plantanalytics.exceptions.RestBadRequestException;
import br.com.fiap.plantanalytics.models.Usuario;
import br.com.fiap.plantanalytics.services.UsuarioService;
...
public class UsuarioController {
    ...
    public ResponseEntity<EntityModel<UsuarioResponseDTO>> cadastrar(@Valid @RequestBody Usuario usuario)
    {
        ...
        var responseDTO = new UsuarioResponseDTO(usuarioCadastrado.getId(), usuarioCadastrado.getUsuario(), usuarioCadastrado.getEmail(), usuarioCadastrado.getSenha());

        var entityModel = EntityModel.of(
                responseDTO,
                linkTo(methodOn(UsuarioController.class).cadastrar(usuario)).withSelfRel(),
                linkTo(methodOn(UsuarioController.class).atualizar(new UsuarioUpdateDTO(usuarioCadastrado.getUsuario(), usuarioCadastrado.getEmail(), usuarioCadastrado.getSenha()), usuarioCadastrado.getId())).withRel("atualizar"),
    ...
    public EntityModel<UsuarioResponseDTO> atualizar(@RequestBody UsuarioUpdateDTO usuario, @PathVariable long id)
    {
        ...
        return EntityModel.of(
                ...
                linkTo(methodOn(UsuarioController.class).cadastrar(new Usuario(usuario.usuario(), usuario.email(), usuario.senha()))).withRel("cadastrar"),
